...
class Role:
    ...
    def __init__(self, game, pname):
        self.game = game
        self.name = "Role"
        self.pname = pname
        self.alignment = None

    #the following methods are required to use Roles as dict keys       
    def __hash__(self):
...
class Vanilla_Role(Role):
    ...
    def action_vote(self, target):
        game.vote.unvote(self.pname, self.votedFor)
        game.vote.voteFor(self.pname, target)
        self.votedFor = target

class Mafia(Vanilla_Role):
    """Standard mafia goon"""
    def __init__(self, game, pname):
        Vanilla_Role.__init__(self, game, pname)
        self.name = "Mafia"
        self.alignment = "scum"
